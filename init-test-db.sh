...
#
# Purpose: Create a separate "keelapp_v2_test" database alongside the default
# "keelapp_v2_dev" database. The test database is used exclusively by the
# Jest integration test suite so that tests never touch development data.

set -e

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    CREATE DATABASE keelapp_v2_test;
EOSQL
