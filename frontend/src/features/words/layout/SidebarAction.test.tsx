...
describe('SidebarAction', () => {
    ...
    it('collapsed: visually hides the label (sr-only) and shows it in a hover tooltip instead', async () => {
        ...
        const label = screen.getByText('Add another translation');
        ...
        expect(screen.getAllByText('Add another translation')).toHaveLength(1);
        await user.hover(screen.getByRole('button', { name: 'Add another translation' }));
        await waitFor(() => expect(screen.getAllByText('Add another translation')).toHaveLength(2));
    });
