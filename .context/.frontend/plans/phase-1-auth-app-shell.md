...
- **Part B still pending:** `e2e/tests/phase-1-auth.spec.ts` is not written yet (only `smoke.spec.ts` exists). Needs `docker:up` + `db:migrate` + both dev servers.
...
- **Still pending:** Part B (`e2e/tests/phase-1-auth.spec.ts`) + the baseline commit.
