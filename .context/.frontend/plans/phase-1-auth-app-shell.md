...
### Outcome — parts A (backend hygiene) + C (gate + docs) done 2026-09-10; part B (e2e spec) pending
...
- **Part B still pending:** `e2e/tests/phase-1-auth.spec.ts` is not written yet (only `smoke.spec.ts` exists). Needs `docker:up` + `db:migrate` + both dev servers. Plus the user has final Phase-1 changes to fold in before the baseline commit.
