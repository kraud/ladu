...
| 5 — backend hygiene + phase gate | 🔨 parts A+C + the final Phase-1 change done 2026-09-10 — leaks closed (`serializeUser` allowlist; both `userColumnsWithoutPassword` trimmed); registration language picker + public UI-language selector added; backend **141/141**, frontend **89/89**, build green; grep gate + guard verified; both new UX pieces verified in-browser. **Pending:** e2e spec (part B) + baseline commit |

Deviations agreed with the user (full text in the plan's Slice 4/5 outcomes): no `VerifyEmailBanner` (unverified users are blocked at login); five shadcn primitives (`dialog`, `alert-dialog`, `checkbox`, `radio-group`, `textarea`) held to Phase 2; Dashboard/metrics (metrics query, stat cards, both word-derived charts) re-scoped to **Phase 3.5** (2026-09-10) — Phase 1 Home is only the welcome banner and ships **no `useQuery`**; `button.tsx` wrapped in `forwardRef` (React-18 ref support, first needed by `sheet.tsx`). **Two blueprint additions** (neither in `ui/01-auth.md`): a language-selection step in registration (`users.languages`, >= 2, selection order; backend-validated) and a UI-language selector on the public routes (persists via the i18next detector cache, and is written to `users.uiLanguage` on the login/register request). Kill-switch (study §11b): 5 slices / 3 sessions, landed on scope, reusable design-system cost paid down — **verdict: continue the rewrite**; re-evaluate if Phase 3's form engine overruns. The Phase 1 e2e spec (`phase-1-auth.spec.ts`) is still outstanding.
