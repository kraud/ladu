...
**`_id` correction (2026-09-08):** the `_id` MongoDB artifact is being removed as-we-go, not in one refactor — see the standing rule in §4. Frontend done now: `ts/interfaces.ts` (`UserData`/`NotificationData`/`FriendshipData`/`TagData`/`FilterItem` → `id`) and `authStore` (`RawUser._id` and the `raw._id` fallback dropped) plus their tests; 42/42 + build green. Backend auth serializers (`serializeUser`/`serializeLoginUser`/`publicUserResponse`) and `authMiddleware` still emit the `_id` alias — stripped in Slice 5 alongside the other auth-controller hygiene, so Slice 3's login/register normalizer must land against the `id`-only responses.
